...
public class DataStreamBuffer {
	...
	public ArrayList<Window> cretaeWindows() {
		...
		while(i<bufferSize) {
			...
			for(int j=0; i<windowSize; j++) {
				// Left
				window.acel_left_x[j] = Acel_Left_X.getData()[i];
				window.acel_left_y[j] = Acel_Left_Y.getData()[i];
				window.acel_left_z[j] = Acel_Left_Z.getData()[i];
				
				window.gyro_left_x[j] = Gyro_Left_X.getData()[i];
				window.gyro_left_y[j] = Gyro_Left_Y.getData()[i];
				window.gyro_left_z[j] = Gyro_Left_Z.getData()[i];
				
				window.mag_left_x[j] = Mag_Left_X.getData()[i];
				window.mag_left_y[j] = Mag_Left_Y.getData()[i];
				window.mag_left_z[j] = Mag_Left_Z.getData()[i];
				
				window.fsr_left_back[j] = FSR_Left_F.getData()[i];
				window.fsr_left_front[j] = FSR_Left_B.getData()[i];
				
				// Right
				window.acel_right_x[j] = Acel_Right_X.getData()[i];
				window.acel_right_y[j] = Acel_Right_Y.getData()[i];
				window.acel_right_z[j] = Acel_Right_Z.getData()[i];
				
				window.gyro_right_x[j] = Gyro_Right_X.getData()[i];
				window.gyro_right_y[j] = Gyro_Right_Y.getData()[i];
				window.gyro_right_z[j] = Gyro_Right_Z.getData()[i];
				
				window.mag_right_x[j] = Mag_Right_X.getData()[i];
				window.mag_right_y[j] = Mag_Right_Y.getData()[i];
				window.mag_right_z[j] = Mag_Right_Z.getData()[i];
				
				window.fsr_right_back[j] = FSR_Right_F.getData()[i];
				window.fsr_right_front[j] = FSR_Right_B.getData()[i];
			}
