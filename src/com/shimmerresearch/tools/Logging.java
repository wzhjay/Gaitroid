	...
	public void logData(ObjectCluster objectCluster){
		...
		try {
			...
			for (int k=0;k<mSensorUnits.length;k++) { 
		        ...
		        else if (mSensorUnits[k]=="i8"){writer.write("");} //center the graph, so the negative values will be displayed
		        else if (mSensorUnits[k]=="u12"){writer.write("");}
		        else if (mSensorUnits[k]=="u16"){writer.write("");}
		        else if (mSensorUnits[k]=="i16"){writer.write("");}        // it is actually a signed 12bit value for magnetometer
		        else {
